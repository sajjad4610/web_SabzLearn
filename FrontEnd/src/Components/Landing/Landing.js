...
import LandingCont from "./LandingCont/LandingCont";
...
export default function Landing() {
  const [allCourse, setAllcourses] = useState(0);
  const [allUser, setAllUser] = useState(0);
  const [addPeriodtime, setAddPeriodtime] = useState(0);
  let h = 0;
  let AddH = 0;
  let m = 0;
  let AddM = 0;
  ...
  useEffect(() => {
    GetServises(GetApi.CourseApi).then((result) => {
      setAllcourses(result.length);
      let periodtime = result.map((e) => {
        index = e.periodtime.indexOf(":");
        h = Number(e.periodtime.substr(0, index) * 60);
        AddH += h;
        m = Number(e.periodtime.substr(index + 1));
        AddM += m;
        Tolal = AddH + AddM;
        return Tolal;
      });
      setAddPeriodtime(periodtime[periodtime.length - 1]);
    });

    GetServises(GetApi.UsersApi).then((result) => {
      setAllUser(result.length);
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  return (
    <section className=" mb-3 landing">
      <div className="container">
            ...
            <Typewriter
              onInit={(Typewriter) => {
                Typewriter.typeString(
                  ...
                  .pauseFor(2000)
                  .deleteAll();
                Typewriter.typeString(`با ما باشید تا با هم پیشرفت کنیم !`)
                  .start()
                  .pauseFor(2000)
                  .deleteAll();
              }}
        ...
        <h2 className="landing__subtitle m-5">
          با آکادمی سبزلرن، برنامه نویسی رو با خیال راحت یاد بگیر و پیشرفت کن.
        </h2>
        <div className="row justify-content-center w-100 me-0 ms-0">
          <div className="col-12 col-md-8  d-flex flex-column align-items-center justify-content-center pe-0 ps-0">
            <SearchBox></SearchBox>
          </div>
        </div>

        <div className="landing-status d-none d-sm-flex">
          <div className="landing-status__item">
            ...
            <LandingCont speed={50} count={allUser} />
          ...
          <div className="landing-status__item">
            ...
            <LandingCont speed={100} count={allCourse} />
          ...
          <div className="landing-status__item">
            ...
            <LandingCont speed={1} count={addPeriodtime / 60} />
            <span className="landing-status__text">
